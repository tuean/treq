...
这个项目（含界面、核心逻辑、测试、这份 README）绝大部分代码由 AI 编程助手（Claude Code / pi）编写，人负责提需求、试用来回打磨、拍板取舍。仓库里 237 个测试用例和每次改动后的截图自检，就是让它敢连续重构的前提。
...
- **树里直接拖拽**：请求/分组拖到别的分组、别的集合，或调同层顺序（分组行上/下 30% 是同级插入、中间是「放进这个分组」，集合行整行都能放；分组拖进自己子孙会被拦下），松手即落盘
...
- 关掉窗口后点 Dock 图标 / 双击 app 会把同一个窗口开回来（连同当前状态，不用重启）
- 自动备份：默认**一天一份、保留 10 份**（工作区同级 `<工作区名>-backups/`），间隔与份数在设置 → 备份里改
- 下拉框 6 种视觉样式可选（设置 → 样式，选中即生效）；**界面/代码字体与字号可调**（空家族＝跟随系统，改完当帧生效）
- 所有纵向滚动区都有滚动条（自绘），**点条带跳转、按住滑块可拖动**，滚轮照旧归内容
